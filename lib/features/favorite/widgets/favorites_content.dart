...
class _FavoritesContentState extends State<FavoritesContent> {
  ...
  // Filter state variables - Multiple choice implementation
      ...
      case 'distance':
        if (_currentPosition != null) {
          ...
          for (int i = 0; i < _filteredDestinations.length; i++) {
            distances[i] = _calculateDistance(_filteredDestinations[i]);
          }
          _filteredDestinations.sort((a, b) {
            final indexA = _filteredDestinations.indexOf(a);
            final indexB = _filteredDestinations.indexOf(b);
            final distA = distances[indexA] ?? 0.0;
            final distB = distances[indexB] ?? 0.0;
    ...
    return SingleChildScrollView(
      child: Column(
        children: [
          ...
          _buildResultsSection(textTheme),
          _buildGrid(),
  ...
  Widget _buildResultsSection(TextTheme textTheme) {
    return Column(
      children: [
        // Results count
        Padding(
  ...
  Widget _buildActiveFilters() {
    return Padding(
      ...
      child: Wrap(
        ...
        children: [
          if (_searchQuery.isNotEmpty)
            _buildFilterChip(Icons.search, '"$_searchQuery"'),
          if (_selectedCategoryIds.isNotEmpty)
            _buildFilterChip(Icons.category, 
              _selectedCategoryIds.length == 1 
                ? _getCategoryName(_selectedCategoryIds.first)
                : '${_selectedCategoryIds.length} kategori'),
          if (_minRating != null)
            _buildFilterChip(Icons.star, '${_minRating!.toStringAsFixed(1)}+'),
          if (_sortBy != 'newest')
            _buildFilterChip(Icons.sort, _getSortDisplayName(_sortBy)),
        ],
      ),
    );
  }
          ...
          Padding(
            ...
            child: Column(
              children: [
                ...
                Text(
                  _searchQuery.isEmpty ? 'Belum Ada Favorit' : 'Tidak Ditemukan',
                  style: textTheme.titleMedium?.copyWith(
                    fontWeight: FontWeight.w600,
          ...
          children: [
            FilterChip(
              ...
              onSelected: (selected) => setModalState(() {
                if (selected) _selectedCategoryIds.clear();
              }),
